...
- **Full-journey integration test (NEW)** — `src/app/__tests__/journey.test.ts` scripts the whole loop against the real validated content corpus + the pure machine/serving/reducers/selectors + an in-memory AsyncStorage: fresh install (Basic-only auto-start, no choice screen) → play b01 (miss one, pass by streak — the wrong-answer history is kept even when correct Review answers clear the weakness mid-level) → play b02 to a **real 12-answer mercy-end** (unlocked but NOT passed, frontier still advances, Weakness Queue fed) → Review groups every miss by rule → a queued rule resurfaces as a Review serve in a later level that carries it → the slice survives a storage round-trip unchanged → reset clears progress while settings survive → relaunch re-auto-starts. Every transition asserts the frontier is a real level id and an ended level is never left resumable — the "no data loss / no invalid navigation" invariant. A second test chains b01 pass → b02 mercy → b03 pass and asserts the frontier + completion invariants.
- **Navigator route-wiring integration tests (NEW)** — `src/navigation/__tests__/AppNavigator.test.tsx` renders the REAL `AppProvider` + `AppNavigator` (native-stack; enabled in Jest by a `transformIgnorePatterns` override in `jest.config.js` for `@react-navigation` / `react-native-screens` / `react-native-safe-area-context`): a Basic-only fresh install boots straight into LevelPlay with StartPoint never rendered (the "only one track bundled" / "returning players are not asked again" route decision), a real 3-correct streak pass routes to the Result screen with the "Streak!" message and the frontier advanced to b02 persisted underneath, "Continue to <next level>" replaces into b02's LevelPlay, and a returning player with saved progress resumes at their current level (b05) with no StartPoint flash.
