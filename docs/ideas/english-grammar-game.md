...
A mastery-based grammar game with 3 tracks as *chapters* (Basic / Intermediate / Advanced), each level a single grammar topic. You pass a level by proving you know the topic — **3 correct in a row, OR 8 total correct, whichever comes first.** Fast players fly through; weak players stay and get taught. Every wrong answer triggers a topic lesson card + the question's "why," then a re-test on the same rule. Missed rules accumulate in a cross-level Weakness Queue that resurfaces in later levels and on a Review screen. On first launch the app asks **"Where do you want to start?"** (Beginner / Some English / Advanced) so skilled users begin at Intermediate or Advanced without the Basic march — the full map stays visible and everything is reachable anytime. Settings: reset progress, theme (device/dark/light), wrong-answer review. Content is AI-generated, hand-reviewed.

**Architecture principle — the app is a player, the content is a database:** the number of tracks and levels is a *content property*, never hardcoded. v1 ships a subset of Basic; later versions add levels and even whole tracks purely by shipping reviewed content data — no code changes.
...
- [ ] Advanced users will trust "start higher" and not feel they're missing content — the visible full map reassures them
...
- 4 screens: level map, level play, settings, start-higher choice
...
- What happens after all 90 levels? (endless review / graduation screen)
