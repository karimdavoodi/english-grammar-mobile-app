...
Task 27B release-regression record for the English Grammar Review. The automated
...
| iOS simulator | `xcodebuild -workspace ios/EnglishGrammarReview.xcworkspace -scheme EnglishGrammarReview -configuration Debug -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO build` | Fresh install → choose a track → play → pass → map → Mixed Review → reset |
| iOS archive | `xcodebuild -workspace ios/EnglishGrammarReview.xcworkspace -scheme EnglishGrammarReview -configuration Release -destination 'generic/platform=iOS' -archivePath build/EnglishGrammarReview.xcarchive archive` | Owner enables signing, uploads to TestFlight, then repeats the iOS simulator flow on a TestFlight build |
